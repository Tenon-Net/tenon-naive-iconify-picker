...
> 任意の数のアイコンライブラリを登録でき(各ライブラリが 1 つのタブになります)、**ネットワークリクエストゼロ**で閲覧し、独自の SVG を差し込み、選択結果を 1 本の文字列として保存できます —— 「このメニューにアイコンを 1 つ選ぶ」といったフィールドに最適です。
...
- 🎨 **テーマ追従** —— ボーダー・文字色・ホバー・プライマリカラー・角丸のすべてが `useThemeVars()` を通じてホストの Naive UI テーマ(ライト / ダーク / カスタムプライマリ)に追従します。CSS 変数の配線は不要です。
- 🖼️ **ローカル SVG** —— プロジェクト独自の SVG を登録し、`local:<name>` として選択できます。
- 🌐 **オンラインの逃げ道** —— 任意の Iconify 名(例:`mdi:home`)を入力すると、接続時にオンラインで読み込みます。
- 🌍 **i18n 対応** —— すべての文言は `labels` prop から供給されます。デフォルトは英語で、独自の翻訳を渡せます(→ [国際化](#国際化-i18n))。
- 🧩 **単一文字列の値** —— `v-model` は 1 本の `prefix:name`(または `local:name`)文字列です。そのまま DB カラムに保存し、どこでも `<OfflineIcon>` で描画できます。
...
`v-model` が保持するのは `lucide:rocket` や `local:star` のような 1 本の文字列です。契約はこれだけです。
...
### `setupIconPicker(options)` —— 任意の一括セットアップ
